...
// Dev-only: log in as a seeded test user without credentials. The backend
// rejects this unless it is running with DEBUG=true.
export async function devLogin(role: string): Promise<void> {
  const data = await graphql<{ devLogin: { token: string } }>(
    `mutation ($role: String) { devLogin(role: $role) { token } }`,
    { role }
  );
  setToken(data.devLogin.token);
}
